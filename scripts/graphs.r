...
load_benchmark <- function(Dir) {
    ## Set maximum number of rows based on duration of benchmark phases
    ## so the results as clients drain off don't skew results
    max_rows = 241
    ...
    requests <- read.delim(sprintf("%s/%s", Dir, "request.txt"),
                           header = FALSE, sep = " ", nrows = max_rows,
...
plot1 <- qplot(elapsed, per_second,
               data = b1$requests,
               color = server,
               geom = "smooth",
               size = I(1.5),
               ...
               ylab = "Requests / sec",
               main = "Throughput")
plot1 <- plot1 + geom_smooth(data = b2$requests, size = 1.5, alpha = 0)
...
plot2 <- qplot(elapsed, X10sec_mean,
               data = b1$cpu,
               color = server,
               geom = "smooth",
               size = I(1.5),
...
plot3 <- qplot(elapsed, X10sec_mean,
               data = b1$mem,
               color = server,
               geom = "smooth",
               size = I(1.5),
